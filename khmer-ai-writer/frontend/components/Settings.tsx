...
export function Settings({ user, onUpdateProfile, onUpgrade }: SettingsProps) {
  ...
  const loadBillingHistory = async () => {
    ...
    try {
      ...
      setBillingHistory(response.billing || []);
    } catch (error) {
  ...
  const billingTotalPages = Math.max(1, Math.ceil(billingHistory.length / billingPageSize));
  const billingStart = (billingPage - 1) * billingPageSize;
  const billingPageItems = billingHistory.slice(billingStart, billingStart + billingPageSize);
            ...
            <div className="space-y-6">
              ...
              <div className="glass p-6 rounded-2xl border border-border">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      {getTierIcon()}
                      <h4 className="text-xl font-bold text-foreground">{user?.tier?.toUpperCase()} Plan</h4>
                    </div>
                    <p className="text-muted-foreground">
                      {user?.tier === 'free' && 'Basic features with limited access'}
                      {user?.tier === 'premium' && '$29/month - Advanced features and priority support'}
                      {user?.tier === 'business' && '$99/month - Enterprise-grade features and unlimited access'}
                    </p>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-border space-y-4">
                  <h5 className="text-sm font-semibold text-muted-foreground">Change Your Plan</h5>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className={`p-4 rounded-xl border-2 transition-all ${
                      ...
                      <p className="text-xs text-muted-foreground mb-3">Basic features</p>
                  {/* Downgrade actions removed for demo */}
                      {user?.tier === 'free' && (
                        <span className="text-xs text-success">Current Plan</span>
                      )}
                    </div>
                    ...
                    <div className={`p-4 rounded-xl border-2 transition-all ${
                      ...
                      <p className="text-xs text-muted-foreground mb-3">$29/month</p>
                      {user?.tier === 'free' && (
                        <button 
                          onClick={() => handleUpgradeTier('premium')}
                          disabled={isUpgrading}
                          className="w-full px-3 py-1.5 text-xs bg-warning/15 hover:bg-warning/20 text-warning rounded-lg transition-colors disabled:opacity-50"
                        >
                  ...
                  {/* Downgrade actions removed for demo */}
                    ...
                    <div className={`p-4 rounded-xl border-2 transition-all ${
                      ...
                      <p className="text-xs text-muted-foreground mb-3">$99/month + Team</p>
                      {user?.tier !== 'business' && (
                        <button 
                          onClick={() => handleUpgradeTier('business')}
                          disabled={isUpgrading}
                          className="w-full px-3 py-1.5 text-xs bg-primary/15 hover:bg-primary/20 text-foreground rounded-lg transition-colors disabled:opacity-50"
                        >
                          {isUpgrading ? 'Processing...' : 'Upgrade'}
                        </button>
                      )}
                  ...
                  <div className="pt-4 border-t border-border">
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-error/15 border border-error/30 text-error rounded-xl hover:bg-error/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      >
              ...
              {/* Payment Method (Mock) */}
              <div className="glass p-6 rounded-2xl border border-border">
                <h4 className="text-lg font-semibold text-foreground mb-4">Payment Method</h4>
                <div className="flex items-center justify-between p-4 bg-card rounded-xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-foreground">KHQR / ABA Bank</p>
              ...
              <div className="glass p-6 rounded-2xl border border-border">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-lg font-semibold text-foreground mb-1">Billing History</h4>
                  ...
                  >
                    {loadingBilling ? 'Loading...' : 'View History'}
                  </button>
                </div>
                ...
                {showBillingHistory && (
                  <div className="mt-6 pt-6 border-t border-border">
                    <div className="flex justify-between items-center mb-4">
                      <h5 className="text-foreground font-medium">Transaction History</h5>
                      <button 
                    ...
                    {billingHistory.length === 0 ? (
                      ...
                      <div className="space-y-3 max-h-96 overflow-y-auto">
                        {billingPageItems.map((item: any) => (
                          <details key={item.id} className="glass p-4 rounded-xl border border-border">
                            <summary className="flex justify-between items-start cursor-pointer list-none">
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">
                                  {item.tier ? `${item.tier} - ${item.billing_cycle}` : 'Payment'}
                                </p>
                              ...
                              <div className="text-right">
                                <Badge 
                                  className={item.status === 'completed' ? 'bg-success/15 text-success' : 'bg-warning/15 text-warning'}
                                >
                                  {item.status}
                                </Badge>
                                <p className="text-xs text-muted-foreground mt-1">
                            ...
                            <div className="mt-4 grid gap-2 text-xs text-muted-foreground">
                              {item.transaction_id && <div>Transaction: {item.transaction_id}</div>}
                              {item.description && <div>Description: {item.description}</div>}
                              {item.billing_cycle && <div>Billing cycle: {item.billing_cycle}</div>}
                              {item.tier && <div>Plan: {item.tier}</div>}
                            </div>
                          </details>
                        ))}
                        <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button
                              ...
                              className="px-3 py-1 text-xs rounded-lg border border-border text-muted-foreground disabled:opacity-40"
                            >
                              Prev
                            </button>
                            <button
